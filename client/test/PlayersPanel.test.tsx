...
describe("PlayersPanel", () => {
  it("lists who is on, with their slot and latency", () => {
    render(<PlayersPanel players={[jeff]} running={true} onRefresh={vi.fn()} onRunCommand={vi.fn()} />);
  ...
  it("says nobody is on rather than showing an empty table", () => {
    render(<PlayersPanel players={[]} running={true} onRefresh={vi.fn()} onRunCommand={vi.fn()} />);
  ...
  it("says the server is stopped, which is not the same as nobody being on", () => {
    render(<PlayersPanel players={[]} running={false} onRefresh={vi.fn()} onRunCommand={vi.fn()} />);
  ...
  it("shows a dash rather than inventing a session length", () => {
    render(<PlayersPanel players={[{ ...jeff, joinedAt: null }]} running={true} onRefresh={vi.fn()} onRunCommand={vi.fn()} />);
    expect(screen.getByText("-")).toBeInTheDocument();
    expect(screen.queryByText(/0m$/)).not.toBeInTheDocument();
  });

  it("shows a dash for a latency it has not been told", () => {
    render(<PlayersPanel players={[{ ...jeff, latency: null }]} running={true} onRefresh={vi.fn()} onRunCommand={vi.fn()} />);
    expect(screen.queryByText(/ms/)).not.toBeInTheDocument();
  });

  it("falls back to the auth when the name is not known yet", () => {
    render(<PlayersPanel players={[{ ...jeff, name: "" }]} running={true} onRefresh={vi.fn()} onRunCommand={vi.fn()} />);
    expect(screen.getByText("76561198048435182")).toBeInTheDocument();
  });

  it("refreshes on request", async () => {
    const onRefresh = vi.fn();
    render(<PlayersPanel players={[jeff]} running={true} onRefresh={onRefresh} onRunCommand={vi.fn()} />);
